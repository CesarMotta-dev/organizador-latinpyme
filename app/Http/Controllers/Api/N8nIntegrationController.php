...
class N8nIntegrationController extends Controller
{
    ...
    {
        $allRules = $rules
            ->filter(fn ($rule) => filled($rule->correo))
            ->values()
            ->map(fn ($rule) => [
                'id' => $rule->id,
                ...
                'carpeta_sugerida' => $rule->carpeta_sugerida ?? '',
                'confirma_sugerencia' => $rule->confirma_sugerencia ?? 'No',
                'carpeta_elegida' => $rule->carpeta_elegida ?? '',
                'id_mensaje' => $rule->id_mensaje,
                'estado' => $rule->estado,
            ]);

        $usableRules = $rules
            ->filter(fn ($rule) => filled($rule->correo) && filled($rule->carpeta) && strtolower(trim($rule->carpeta)) !== 'por clasificar')
            ->values();
